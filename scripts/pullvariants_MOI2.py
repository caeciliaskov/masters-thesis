...
##################################NEW LINE######################################
outgroup_poly_sites_file = "{sandbox}/out/1KGP_HGDP/1KGP_HGDP_chr{chrom}_der.txt".format(sandbox = sandbox, chrom = chrom)
out_filt = vpf.get_outgroup_poly_sites_filt(outgroup_poly_sites_file, chrom, callset, all_filt, max_min_anc_filt)
print("out_filt", out_filt.shape)

################################################################################

pos = callset["{chrom}/variants/POS".format(chrom = chrom)][:][all_filt][max_min_anc_filt][out_filt]
...
#########################################################
    ...
    #########################################################
            ...
            d = {'pop1': pop1_list, 'pop2': pop2_list, 'mpd_{}'.format(anc1): mpd_list, 'div_{}'.format(anc1):d_list}
            df = pd.DataFrame(data=d)
            df.to_csv("{}/{}/divergence/div_{}_{}_{}_{}_{}_{}_{}_chr{}_AFR.txt".format(results,anc1, data1, pop1, anc1, data2, pop2, anc2, mean_prob, chrom), sep='\t', index=False, header=None)

            with open(os.path.join("{}/{}/completed".format(results,anc1), 'div_{}_{}_{}_{}_{}_{}_{}_chr{}_AFR.DONE'.format(data1, pop1, anc1, data2, pop2, anc2, mean_prob, chrom)), 'w') as fp:
                pass
        else:
            pop_list = [pop1, pop2]
            pop1_list = []
